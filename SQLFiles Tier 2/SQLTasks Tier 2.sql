...
ELSE 'cheap' END AS maintenance_cat
FROM Facilities; 
...
SELECT DISTINCT CONCAT(M.firstname, ' ', M.surname, ' ', F.name) AS member_and_court
FROM Members AS M
LEFT JOIN Bookings AS B ON M.memid = B.memid
LEFT JOIN Facilities AS F ON B.facid = F.facid
WHERE F.name IN ('Tennis Court 1', 'Tennis Court 2')
AND M.firstname <> 'GUEST'
ORDER BY member_and_court;

/*In Jupyter Notebook-sqlite*/
query7 = """
